...
# -*- coding: utf-8 -*-
"""
FUTBIN FUT25 – SCRAPER con colonna “Revision”
"""
...
COLS = [
    ...
    # info varie
    "Position","Weak foot","Skill moves","Preferred foot",
    "Height","Weight","Alternative positions","Age",
    "Nation","League","Team","play style","url",
    # GK
...
# ───────── helper brevi
html = lambda u: BeautifulSoup(scraper.get(u,timeout=30).raise_for_status() or
                               scraper.get(u).text,"lxml")
...
def parse_row(tr:Tag)->tuple[str,dict,str]:
    ...
    base = {
        "Name": txt(a),
        "OVR":  txt(tr.select_one("div.player-rating-card-text")),
        "url":  link,
...
def parse_detail(url:str)->Dict:
    ...
    if tbl:
        for row in tbl.select("tr"):
            d[txt(row.th)] = txt(row.td)

    # Height / Weight / Preferred foot / Age / Revision
    ...
    for row in s.select("div.player-stat-row"):
        ...
        val  = vtag.get("data-stat-value") or txt(vtag)
        if name=="Dribbling":
            key="DRI" if vtag.get("data-stat-id")=="4" else "Dribbling"
        else:
